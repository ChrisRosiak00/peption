...
import { Bell, Compass, Heart, Home, LineChart, MessageCircle, Plus, Sparkles, User, Users } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-border bg-surface/80 backdrop-blur-md px-4 md:px-6 h-14 lg:h-16">
      ...
      <div className="hidden lg:block text-sm text-ink-500">
        Welcome back — here is your journey today.
      </div>
    ...
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 border-t border-border bg-surface/90 backdrop-blur-xl">
      <div className="flex items-center justify-around px-2 pt-2 pb-[max(env(safe-area-inset-bottom),12px)]">
        ...
        <Link
          ...
          className="relative -mt-5 flex items-center justify-center size-12 rounded-full shadow-[0_12px_30px_-10px_rgba(124,58,237,0.7)] [background:linear-gradient(135deg,#a78bfa_0%,#8b5cf6_50%,#ec4899_120%)] text-white"
        >
          <Plus size={20} />
        </Link>
